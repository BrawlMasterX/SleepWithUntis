	...
	public Period(JSONObject json) {
		...
		if(arr != null) {
			for (int i = 0; i < arr.length(); i++) {
				rooms.add(new Room(arr.optJSONObject(i)));
			}
		}
		...
		if(arr != null) {
			for (int i = 0; i < arr.length(); i++) {
				klassen.add(new Klasse(arr.optJSONObject(i)));
			}
		}
		...
		if(arr != null) {
			for (int i = 0; i < arr.length(); i++) {
				subjects.add(new Subject(arr.optJSONObject(i)));
			}
		}
		...
		if(arr != null) {
			for (int i = 0; i < arr.length(); i++) {
				teachers.add(new Teacher(arr.optJSONObject(i)));
			}
		}
	}
